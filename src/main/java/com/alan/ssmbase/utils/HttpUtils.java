package com.alan.ssmbase.utils;
...
public class HttpUtils {
     ...
     * @throws IOException
     */
    public HttpResponse sendPost(String urlString, String data, Map<String, String> headers) throws IOException {
        cookies.clear();
        try {
            HttpURLConnection urlConnection = (HttpURLConnection) sendPostRequest(urlString, data, headers);
            HttpResponse respons = makeContent(urlString, urlConnection);
            if (respons.code == 302) {
                if (respons.cookies != null) {
                    cookies.addAll(respons.cookies);
                }
                return sendPost(respons.location, data, headers);
            } else {
                if (cookies.size() != 0) {
                    respons.cookies.addAll(cookies);
                }
                return respons;
            }
        } catch (Exception e) {
     ...
     * @throws IOException
     */
    public HttpResponse sendPost(String urlString, Map<String, String> headers) throws IOException {
        cookies.clear();
        return this.send(urlString, "POST", null, headers);
    }
    ...
    private HttpResponse send(String urlString, String method, Map<String, String> parameters,
        ...
        HttpResponse respons = makeContent(urlString, urlConnection);
        if (respons.code == 302 || respons.code == 301) {
            if (respons.cookies != null) {
                cookies.addAll(respons.cookies);
            }
            return send(respons.location, method, parameters, propertys);
        } else {
            if (cookies.size() != 0) {
                respons.cookies.addAll(cookies);
            }
    ...
    private HttpResponse makeContent(String urlString, HttpURLConnection urlConnection) throws IOException {
        ...
        try {
            ...
            httpResponser.location = urlConnection.getHeaderField("Location");
            httpResponser.message = urlConnection.getResponseMessage();
            httpResponser.contentType = urlConnection.getContentType();
            httpResponser.method = urlConnection.getRequestMethod();
            httpResponser.connectTimeout = urlConnection.getConnectTimeout();
            httpResponser.readTimeout = urlConnection.getReadTimeout();
            httpResponser.cookies = urlConnection.getHeaderFields().get("Set-Cookie");
     ...
     * @throws Exception
     */
    public static URLConnection sendPostRequest(String url, String data, Map<String, String> headers)
            throws Exception {
        Set<Entry<String, String>> entrys = null;
        // 如果存在参数，则放在HTTP请求体，形如name=aaa&age=10
